...
router.get('/create-report', (req, res, next) => {
  ...
  if(name != 'Employee name' && year != 'year' && month != 'month') {
    ...
    .then(theEmployee => {
      ...
      return MonthlyReport.findOne({employee_id, year, month})  })
      .then(theReport => {
        if (theReport !== null) {
...
router.post('/create-report', (req,res, next) => {
  ...
  Employee.findOne({name})
  .then(theEmployee => employee_id = theEmployee._id)  
  ...
  .then(theReport => {
    ...
    return MonthlyReport.findOneAndUpdate({year, month}, {total}, {new:true})   })
  .then(theReport => res.render('reports/report', {theReport, name}))
...
router.post('/update' , (req,res,next) => {
  ...
  .then(theReport => {
      let total = getTotalfromDB(theReport.dailies)
      return MonthlyReport.findOneAndUpdate({_id: theReport._id}, {total}, {new:true}) })
  .then(report => {
    theReport = report
    return Employee.findById(theReport.employee_id)})
  .then(theEmployee => {
